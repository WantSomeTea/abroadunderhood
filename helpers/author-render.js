...
const tweetLink = (tweet) =>
  `https://twitter.com/${underhood}/status/${tweet.id_str}`;
...
const authorIndex = (author) =>
  findIndex(propEq('authorId', author.authorId))(authorsToPost);
const isFirstAuthor = (author) =>
  authorIndex(author) === dec(length(authorsToPost));
const isLastAuthor = (author) =>
  author.authorId === prop('authorId', head(authorsToPost));
const nextAuthor = (author) => {
  if (!isLastAuthor(author))
    return nth(dec(authorIndex(author)), authorsToPost);
};
...
const filterTimeline = (item) =>
  item.text[0] !== '@' || item.text.indexOf(`@${underhood}`) === 0;
const fullText = (item) => {
